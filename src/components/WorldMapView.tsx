...
export function WorldMapView({ projectId }: WorldMapViewProps) {
  ...
  const saveDragEnd=useCallback(()=>{
    ...
    const newX=clamp(pos.x/100,0,1),newY=clamp(pos.y/100,0,1);
  ...
  const handleLocPointerMove=useCallback((e:React.PointerEvent)=>{
    ...
    const newX=clamp(pt.x/100,0,1),newY=clamp(pt.y/100,0,1);
  ...
  const handleBlankClick=useCallback((e:React.MouseEvent)=>{
    ...
    const mx=clamp(pt.x/100,0,1),my=clamp(pt.y/100,0,1);
  ...
  const isDraggingSelected=!!(draggingLocId&&selectedId===draggingLocId&&dragTempPos);
  const panelX=isDraggingSelected?Math.round(dragTempPos!.x):selectedLoc?Math.round(selectedLoc.mapX*100):0;
  const panelY=isDraggingSelected?Math.round(dragTempPos!.y):selectedLoc?Math.round(selectedLoc.mapY*100):0;
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        {locations.length===0?(
          ...
          {isEditMode?<div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 px-4 py-2 text-center text-sm text-muted-foreground">选择或拖动一个地点来调整地图位置。编辑模式下点击地点选中，拖动地点移动位置。</div>:null}
          ...
          {isEditMode&&selectedLoc?<div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2">
            <div className="min-w-0"><span className="text-sm font-medium">{selectedLoc.entry.title||"未命名地点"}</span><span className="ml-2 text-xs text-muted-foreground">X {panelX} · Y {panelY}</span></div>
