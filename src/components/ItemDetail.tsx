...
import { Edit3, Gem, Pin, Star } from "lucide-react";
import type { Entry, ItemProfile } from "@/types";
import { ENTRY_TYPE_LABELS, ITEM_CATEGORY_LABELS, ITEM_STATUS_LABELS } from "@/types";
...
export function ItemDetail({
  ...
  const profile: ItemProfile | undefined = entry.itemProfile;

  const ownerCharacter = useMemo(
    () =>
      profile?.ownerCharacterId
        ? projectEntries.find(
            (e) => e.id === profile.ownerCharacterId && e.type === "character",
          )
        : undefined,
    [profile?.ownerCharacterId, projectEntries],
  );
  ...
  const currentLocation = useMemo(
    () =>
      profile?.currentLocationId
        ? projectEntries.find(
            (e) => e.id === profile.currentLocationId && e.type === "location",
          )
        : undefined,
    [profile?.currentLocationId, projectEntries],
  );

  const creatorFaction = useMemo(
    () =>
      profile?.creatorFactionId
        ? projectEntries.find(
            (e) => e.id === profile.creatorFactionId && e.type === "faction",
          )
        : undefined,
    [profile?.creatorFactionId, projectEntries],
  );
  ...
  const hasProfile =
    ...
    (!!profile.itemCategory ||
      ...
      !!profile.function.trim() ||
      !!profile.materials.trim() ||
      !!profile.origin.trim() ||
  ...
  const hasBasicInfo =
    !!profile?.itemCategory || !!profile?.status;
  ...
  const hasOriginSection =
    (!!profile?.origin.trim());

  const hasHistorySection =
    (!!profile?.history.trim());

  const hasLimitationSection =
    (!!profile?.limitations.trim());
        ...
        {entry.coverImage ? (
          ...
          <div className="border-b border-border/80 bg-gradient-to-b from-primary/5 to-transparent p-5">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <Badge variant="outline">物品档案</Badge>
              {entry.isPinned ? (
          ...
          <article className="space-y-6 p-5">
            ...
            {hasProfile ? (
              <>
                  ...
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    ...
                    <dl className="space-y-2">
                      {profile?.itemCategory ? (
                        <InfoRow
                          label="物品类型"
                          value={ITEM_CATEGORY_LABELS[profile.itemCategory] || profile.itemCategory}
                        />
                      ) : null}
                      {profile?.status ? (
                        <InfoRow
                          label="当前状态"
                          value={ITEM_STATUS_LABELS[profile.status] || profile.status}
                        />
                      ) : null}
                    </dl>
                  </section>
                ...
                {hasAttributionSection ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    ...
                    <dl className="space-y-2">
                      <InfoRow
                        label="当前持有者"
                        value={ownerCharacter?.title ?? ""}
                        onClick={ownerCharacter ? () => onSelectEntry(ownerCharacter) : undefined}
                      />
                      <InfoRow
                        label="当前所在地"
                        value={currentLocation?.title ?? ""}
                        onClick={currentLocation ? () => onSelectEntry(currentLocation) : undefined}
                      />
                      <InfoRow
                        label="所属组织"
                        value={creatorFaction?.title ?? ""}
                        onClick={creatorFaction ? () => onSelectEntry(creatorFaction) : undefined}
                      />
                    </dl>
                  </section>
                ) : null}
                ...
                {/* 外观与功能 */}
                {hasAppearanceSection ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    <h3 className="mb-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      外观与功能
                    </h3>
                    <div className="space-y-4">
                      <ProfileBlock title="外观描述" text={profile?.appearance ?? ""} />
                      <ProfileBlock title="功能 / 用途" text={profile?.function ?? ""} />
                      <ProfileBlock title="材质 / 构造" text={profile?.materials ?? ""} />
                    </div>
                  </section>
                ) : null}

                {/* 来源与历史 */}
                {hasOriginSection || hasHistorySection ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    <h3 className="mb-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      来源与历史
                    </h3>
                    <div className="space-y-4">
                      <ProfileBlock title="来源 / 出处" text={profile?.origin ?? ""} />
                ...
                {/* 限制与备注 */}
                {hasLimitationSection || hasCreatorNotes ? (
                  <section className="rounded-xl border border-border/70 bg-card/40 p-4">
                    <h3 className="mb-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      限制与备注
                    </h3>
                    <div className="space-y-4">
                      <ProfileBlock title="限制 / 副作用" text={profile?.limitations ?? ""} />
                      {hasCreatorNotes ? (
            ...
            <div className="border-t border-border/60 pt-4 text-xs text-muted-foreground">
              <p>
                {ENTRY_TYPE_LABELS.item} · 创建于 {formatDate(entry.createdAt)}
              </p>
